  ...
  stages {
    stage('Build and run tests') {
      steps {
        ...
        sh 'docker-compose -f dev/docker-compose.yaml -p mage run node sh -c \'npm i && npm t\''
        sh 'docker-compose -f dev/docker-compose.yaml -p mage run node sh -c \'npm i && npm run e2e:ci\''
      }
    }
  }
